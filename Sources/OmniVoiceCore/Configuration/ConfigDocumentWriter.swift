...
enum ConfigDocumentWriter {
    ...
    private static func appendSystemASR(
        ...
        appendExternalASR(config.systemASRSettings.externalASR, text: text, to: &lines)
        lines.append("  },")
    }

    private static func appendExternalASR(
        _ settings: ExternalASRSettings,
        text: ConfigDocumentText,
        to lines: inout [String]
    ) {
        lines.append("    \(jsonString(ConfigSchema.SystemASR.externalASR)): {")
        addComment(text.externalASRProviderID, indent: 6, to: &lines)
        lines.append("      \(jsonString(ConfigSchema.ExternalASR.providerID)): \(jsonString(settings.providerID ?? ""))")
        lines.append("    }")
    }
